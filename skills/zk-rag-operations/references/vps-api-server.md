...
### /api/docs serves local Swagger UI (no CDN)
Swagger UI is served from local files (`/data/rag/docs/swagger/`). No external CDN needed.
- `/api/docs` → serves `/data/rag/docs/api-docs.html` (custom local HTML)
- `/swagger/` → static files from `/data/rag/docs/swagger/`
- `/openapi.json` → nginx proxies to `http://127.0.0.1:8100/openapi.json`
- CSP is strict: no external script sources

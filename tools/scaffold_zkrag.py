...
# Paths on the live system — set via environment variables
# $ZK_REPO  → private repo root  (default: $ZK_HOME/zk-rag-v2)
# $ZK_DATA  → data directory (default: ./data)
REPO_SRC   = Path(os.environ.get("ZK_REPO",  "$ZK_REPO"))
DATA_SRC   = Path(os.environ.get("ZK_DATA",  "$ZK_DATA"))
...
_REPLACEMENTS = [
    ...
    ("/data/military-documents",   "$DATA_DIR"),
    ("/data/military-documents/",  "$DATA_DIR/"),
    # Hostnames / usernames
    ("<USER>",                   "youruser"),
    ...
    ("0xbabc60ed17e6387aedab112e80744aa19efcb723", "YOUR_WALLET_ADDRESS"),
    # Hardcoded paths — replace with templated placeholders
    (re.compile(r"/home/\w+/"),                         "$ZK_RAG_HOME/"),
    (re.compile(r"/data/military-documents"),          "$ZK_RAG_DATA_DIR"),
]
...
def scan_for_leaks(root: Path) -> list[tuple[str, str]]:
    ...
    leak_patterns = [
        ...
        (re.compile(r"/home/blockops/"),                              "<HOME>"),
        (re.compile(r"/data/military-documents"),                     "<DATA>"),
        (re.compile(r"b28e65[a-f0-9]+"),                              "Kurier API key fragment"),
        (re.compile(r"blockops"),                                      "<USER>"),
    ]
...
"""
    ...
    print("  Then: review .env.example, fill in keys, and test!")

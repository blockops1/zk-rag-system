...
export function init() {
	...
	const heroProvBtn = document.getElementById("heroSearchProvenanceBtn");
	if (heroBtn && heroInput) {
		heroBtn.addEventListener("click", () => {
			const q = heroInput.value.trim();
			if (q) {
				// Mirror into the main search box so results render in the right place
	...
	if (heroProvBtn && heroInput) {
		heroProvBtn.addEventListener("click", () => {
			const q = heroInput.value.trim();
			if (q) {
				handleSearchProvenance(q);
				document.getElementById("resultsContainer")?.scrollIntoView({ behavior: "smooth" });
			}
		});
	}

	// ── Prompt cards (click to fill hero input and search) ─────────
	document.querySelectorAll(".prompt-card").forEach(card => {
		card.addEventListener("click", () => {
			const q = card.dataset.query || card.textContent.trim();
			if (!q) return;
			const heroInput2 = document.getElementById("heroSearchInput");
			if (heroInput2) {
				heroInput2.value = q;
				heroInput2.scrollIntoView({ behavior: "smooth", block: "center" });
				// Auto-fire a normal search after a brief pause
				setTimeout(() => {

...
Website served from git-managed source (`./website/`) via OpenResty on port 80. All API endpoints verified working:
...
- Fixed `military-manuals-local.conf`: `root` → `./website/`, removed invalid `more_clear_headers`
- Fixed `rag-api.conf`: commented out broken `access_by_lua_block` (was causing 2026-04-14 failure)
- Fixed nginx worker permissions: `chmod o+x /home/blockops` + `./`
- Reloaded via `sudo systemctl reload openresty`

**Problem resolved — website path migration:**
- Old: `./data/website/docs/` (not git-managed, stale)
- New: `./website/` (git-managed, canonical)
...
6. **Local dev server — 2026-04-17** ✅
   - Website copied from VPS: `./data/website/docs/`
...
|------|----------|-----------|
| OpenResty proxy (local dev) | Bypass OpenResty; API server binds directly to `127.0.0.1:8100` | Local dev machine has no OpenResty. Clients hit `http://your-server-ip:8100` directly over HTTP. Production adds OpenResty + HTTPS + auth at the VPS layer. |
...
- Library: `rank_bm25`
- Index file: `./data/bm25_index.pkl`
...
- [x] **D-local — Local dev server** (2026-04-17)
  - [x] Website copied from VPS to `./data/website/docs/`

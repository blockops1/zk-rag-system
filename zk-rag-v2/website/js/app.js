...
export function renderResults() {
	...
	_resultsContainer.querySelectorAll(".passage-card").forEach(async (card) => {
	...
	try {
		...
		if (!chunks || chunks.length === 0) {
			...
			if (navBtn) {
				...
				navBtn.textContent = direction < 0 ? "← Prev Chunk" : "Next Chunk →";
			}
			return;
		}
		...
		if (fetchZkProof) {
			...
			try {
				...
				if (zk) {
					...
					console.warn("[_handleNav] ZK proof was null/falsy — NOT caching");
				}
			} catch (e) {
				console.error("[_handleNav] ZK proof fetch failed:", e.message);
			}
		} else {
			console.log("[_handleNav] fetchZkProof was null — skipping ZK fetch");
		}
...
export async function _autoPollKurier(chunkId, kurierJobId) {
	...
	const isFailed = /failed|rejected|invalid/i.test(cached.kurier_status || "");
...
export async function verifyOnChain(chunkId) {
	...
	const cached = getZkCache(chunkId);
	if (!cached) {
		console.error("[verifyOnChain] No proof in cache for chunkId:", chunkId);
		alert(
	...
	if (cached.kurier_job_id) {
		console.log("[verifyOnChain] polling existing job:", cached.kurier_job_id);
		if (badge) {
...
async function _pollKurierJob(chunkId, jobId, cached) {
	console.log("[_pollKurierJob] START chunkId:", chunkId, "jobId:", jobId);
	const badge = document.getElementById(`zk-status-${CSS.escape(chunkId)}`);

	for (let i = 0; i < 75; i++) {
		// 75 * 4s = 300s max
		await new Promise((r) => setTimeout(r, 4000));
		const st = await getProofStatus(jobId);
		console.log("[_pollKurierJob] poll iteration", i, "status:", st.status);
		...
		inFlight.map(async ([chunkId, proof]) => {
			try {
				...
				if (TERMINAL_KURIER_STATUSES.has(st.status)) {
					...
					const isFailed = /failed|rejected|invalid/i.test(st.status || "");
					_showToast(
						isFailed
...
export function showHowItWorksModal(chunkId) {
	...
	const blockUrl = cached
		? _blockExplorerUrl(
				cached.evm_block_number || cached.public_inputs?.ingestion_block,
			)
		: "";
...
function wireZKBadges() {
	...
	console.log("[wireZKBadges] called, .zk-status-badge count:", badges.length);
	...
	badges.forEach((badge) => {
		badge.addEventListener("click", async (e) => {
			...
			// If already verified or in a terminal state, open results modal
			const cached = getZkCache(chunkId);
			if (
			...
			// Generate proof + auto-submit
			badge.textContent = "⏳ Generating…";
			badge.style.color = "#64b5f6";
			badge.style.cursor = "wait";

			try {
				const proof = await fetchZKProof(docId, chunkId, collection);
...
async function _loadImage(docId, pageNum, cardOrContainer) {
	try {
		...
		const imageContainer = cardOrContainer.classList?.contains("passage-images")
			? cardOrContainer
  ...
  `);
	...
	document.getElementById("payUsdcBtn")?.addEventListener("click", async () => {
		...
		try {
			...
			const { fullMessage, message, validAfter, validBefore, nonceHex } =

...
Binary: ./zk-circuit/target/release/prove
Tree JSONs: ./data/merkleTrees/{doc_id}_tree.json
"""
...
PROVE_BINARY = Path("./zk-circuit/target/release/prove")
...
def load_tree_json(doc_id: str) -> dict:
    ...
    Path: ./data/merkleTrees/{doc_id}_tree.json
    ...
    """
    path = Path(f"./data/merkleTrees/{doc_id}_tree.json")
    if not path.exists():
